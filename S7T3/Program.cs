...
// на определенное число. Метод принимает на вход три числа (n - количество строк матрицы, m - количество столбцов 
// матрицы, k - число, на которое увеличивается каждый новый элемент) и возвращает матрицу, удовлетворяющую этим условиям.
...
// Метод PrintListAvr принимает одномерный массив, возвращенный методом FindAverageInColumns и выводит этот список на 
// экран в формате
...
public class Answer {
    ...
    double sum;
    int index=matrix.GetLength(0)/2;
    if (matrix.GetLength(0)%2==0){
      for (int i = 0; i < matrix.GetLength(1); i++) {
        ...
        for (int j = 0; j < matrix.GetLength(1); j++) {
            array[j]=Convert.ToDouble(matrix[index, j]);      
        }
     }
     return array;
    }
